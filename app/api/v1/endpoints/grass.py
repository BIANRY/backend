...
def sync_grass(
    ...
    if user.last_grass_sync is not None:
        ...
        last_sync = user.last_grass_sync.replace(
            tzinfo=timezone.utc) if user.last_grass_sync.tzinfo is None else user.last_grass_sync
...
def get_grass_by_handle(
        handle: str,
        year: int = Query(..., description="조회할 연도 (예: 2026)"),
):
    """
    백준 ID로 특정 연도의 잔디 데이터를 GAS 호환 형식으로 반환합니다.
    DB 없이 백준 페이지에서 직접 크롤링합니다.
    """
    grass_list = fetch_baekjoon_grass(handle)

    if not grass_list:
        raise HTTPException(status_code=404, detail="데이터를 가져오지 못했습니다.")

    return {
        "grass": [
            {"date": str(item["date"]), "value": item["solved_count"]}
